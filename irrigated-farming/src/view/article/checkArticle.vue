      ...
      <div class="body-content">
        <Form :model="articleForm" :label-width="150" ref="articleForm" style="margin-top: 30px">
          ...
          <Row type="flex" justify="center">
            ...
            <FormItem label="推送方式" >
              <div style="float: left;margin-right: 20px" v-show="recommend">
                每日推荐
              </div>
              <div style="float: left;margin-right: 20px" v-show="information">
                农业资讯
              </div>
              <Button  class="btn-list" @click="creatRecommendation" v-show="!recommend">添加到每日推荐</Button>
              <Button  class="btn-list" @click="creatInformation" v-show="!information">添加到农业资讯</Button>
              <Button  class="btn-list" @click="creatNotice">添加通知</Button>
            </FormItem>
            <FormItem >
...
<script>
  ...
  import { getArticleDetail, uploadImage, creatInformationRecommendation } from '../../api/api'
  export default {
    components: {
      VueEditor
    },
    ...
    methods: {
      ...
      creatNotice () {
        this.$router.push({name: 'notice_article', params: {id: this.$route.params.id}})
      },
      creatRecommendation () {
        let par = {
